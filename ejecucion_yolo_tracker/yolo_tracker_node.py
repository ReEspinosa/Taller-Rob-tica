...
class YOLOTracker(Node):
    def __init__(self):
        ...
        self.model = YOLO('./best.pt')  # Ruta a tu modelo entrenado
    ...
    def image_callback(self, msg):
        try:
            ...
            # Detección YOLO - FORZANDO CPU AQUÍ
            # Agregamos device='cpu' para evitar el error de CUDA
            results = self.model(cv_image, conf=0.5, verbose=False, device='cpu')
...
def main(args=None):
    ...
    # Manejo de excepciones para un cierre limpio (esto arregla el error rcl_shutdown al cerrar)
    try:
        node = YOLOTracker()
        rclpy.spin(node)
        ...
        # Verificamos si el contexto sigue válido antes de cerrar
        if rclpy.ok():
            rclpy.shutdown()
        # El destroy_node debería ir aquí si el nodo fue creado
